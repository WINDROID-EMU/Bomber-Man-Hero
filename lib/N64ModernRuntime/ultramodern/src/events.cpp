...
static struct {
    struct {
        ...
        int field;
        ViState states[2] = {
            ViState{ .mode = &dummy_mode, .control = dummy_mode.comRegs.ctrl },
            ViState{ .mode = &dummy_mode, .control = dummy_mode.comRegs.ctrl }
        };
        ultramodern::renderer::ViRegs regs;
        ...
        void update_vi() {
            ...
            int valid_field = (field >= 0 && field < 2) ? field : 0;
            const OSViCommonRegs* common_regs = &next_mode->comRegs;
            const OSViFieldRegs* field_regs = &next_mode->fldRegs[valid_field];
            PTR(void) framebuffer = osVirtualToPhysical(next_state->framebuffer);
            PTR(void) origin = framebuffer + (field_regs ? field_regs->origin : 0);
