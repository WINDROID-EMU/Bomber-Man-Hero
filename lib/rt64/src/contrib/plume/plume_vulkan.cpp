...
#   elif defined(PLUME_SDL_VULKAN_ENABLED) || defined(RT64_SDL_WINDOW_VULKAN) || defined(__ANDROID__)
        VulkanInterface *renderInterface = commandQueue->device->renderInterface;
...
#   endif
        ...
        // Make sure maxImageCount is handled correctly (0 means no limit).
        if (surfaceCapabilities.maxImageCount > 0) {
            ...
            this->desc.textureCount = std::max(desc.textureCount, surfaceCapabilities.minImageCount);
        }
        ...
        if (compatibleSurfaceFormats.empty()) {
            fprintf(stderr, "Requested surface format not explicitly supported, falling back to available surface formats.\n");
            compatibleSurfaceFormats = surfaceFormats;
        }
    ...
    bool VulkanSwapChain::isEmpty() const {
        return (vk == VK_NULL_HANDLE) || (width == 0) || (height == 0) || textures.empty();
    }
...
#if PLUME_SDL_VULKAN_ENABLED || defined(__ANDROID__)
...
#   if defined(PLUME_SDL_VULKAN_ENABLED) || defined(RT64_SDL_WINDOW_VULKAN) || defined(__ANDROID__)
