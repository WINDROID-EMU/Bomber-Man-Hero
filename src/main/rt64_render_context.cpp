...
#define BMHERO_ANDROID_RT64_LOG(...) __android_log_print(ANDROID_LOG_INFO, "BMHeroRT64", __VA_ARGS__)
#else
#define BMHERO_ANDROID_RT64_LOG(...)
#endif
...
recompui::renderer::RT64Context::RT64Context(uint8_t* rdram, ultramodern::renderer::WindowHandle window_handle, bool debug) {
    BMHERO_ANDROID_RT64_LOG("RT64Context begin rdram=%p debug=%d", rdram, debug ? 1 : 0);
    ...
    if (const char* app_folder_path = std::getenv("APP_FOLDER_PATH")) {
        ...
        BMHERO_ANDROID_RT64_LOG("RT64 dataPath=%s", appConfig.dataPath.string().c_str());
    }
#endif

    // Create the RT64 application.
    BMHERO_ANDROID_RT64_LOG("creating RT64 application");
    app = std::make_unique<RT64::Application>(appCore, appConfig);
    BMHERO_ANDROID_RT64_LOG("created RT64 application");
...
#endif
    BMHERO_ANDROID_RT64_LOG("calling app->setup");
    setup_result = map_setup_result(app->setup(thread_id));
    BMHERO_ANDROID_RT64_LOG("app->setup result=%d chosen_api=%d", static_cast<int>(setup_result), static_cast<int>(app->chosenGraphicsAPI));
    // Get the API that RT64 chose.
    chosen_api = map_graphics_api(app->chosenGraphicsAPI);
    if (setup_result != ultramodern::renderer::SetupResult::Success) {
        BMHERO_ANDROID_RT64_LOG("RT64 setup failed");
        app = nullptr;
        return;
    }

    // Set the application's fullscreen state.
    BMHERO_ANDROID_RT64_LOG("setting fullscreen");
    ...
    BMHERO_ANDROID_RT64_LOG("RT64Context ready sample_positions=%d high_precision=%d", sample_positions_supported ? 1 : 0, high_precision_fb_enabled ? 1 : 0);
}

recompui::renderer::RT64Context::~RT64Context() = default;

void recompui::renderer::RT64Context::send_dl(const OSTask* task) {
...
void recompui::renderer::RT64Context::send_dummy_workload(uint32_t fb_address) {
}

void recompui::renderer::RT64Context::update_screen() {
    app->updateScreen();
}

void recompui::renderer::RT64Context::shutdown() {
    if (app != nullptr) {
        app->end();
    }
}

bool recompui::renderer::RT64Context::update_config(const ultramodern::renderer::GraphicsConfig& old_config, const ultramodern::renderer::GraphicsConfig& new_config) {
...
void recompui::renderer::RT64Context::enable_instant_present() {
...
uint32_t recompui::renderer::RT64Context::get_display_framerate() const {
    return app->presentQueue->ext.sharedResources->swapChainRate;
}

float recompui::renderer::RT64Context::get_resolution_scale() const {
...
void recompui::renderer::RT64Context::check_texture_pack_actions() {
...
RT64::UserConfiguration::Antialiasing recompui::renderer::RT64MaxMSAA() {
    return device_max_msaa;
}

std::unique_ptr<ultramodern::renderer::RendererContext> recompui::renderer::create_render_context(uint8_t* rdram, ultramodern::renderer::WindowHandle window_handle, bool developer_mode) {
    return std::make_unique<recompui::renderer::RT64Context>(rdram, window_handle, developer_mode);
}

bool recompui::renderer::RT64SamplePositionsSupported() {
    return sample_positions_supported;
}

bool recompui::renderer::RT64HighPrecisionFBEnabled() {
    return high_precision_fb_enabled;
}

void recompui::renderer::trigger_texture_pack_update() {
    texture_pack_action_queue.enqueue(TexturePackUpdateAction{});
}

void recompui::renderer::enable_texture_pack(const recomp::mods::ModContext& context, const recomp::mods::ModHandle& mod) {
    ...
    auto find_it = config_schema.options_by_id.find(recompui::renderer::special_option_texture_pack_enabled);
    if (find_it != config_schema.options_by_id.end()) {
        ...
        if (is_texture_pack_enable_config_option(config_option, false)) {
            ...
            if (value) {
                recompui::renderer::secondary_enable_texture_pack(mod.manifest.mod_id);
            }
            else {
                recompui::renderer::secondary_disable_texture_pack(mod.manifest.mod_id);
            }
        }
    }
}

void recompui::renderer::disable_texture_pack(const recomp::mods::ModHandle& mod) {
    texture_pack_action_queue.enqueue(TexturePackDisableAction{mod.manifest.mod_id});
}

void recompui::renderer::secondary_enable_texture_pack(const std::string& mod_id) {
    texture_pack_action_queue.enqueue(TexturePackSecondaryEnableAction{mod_id});
}

void recompui::renderer::secondary_disable_texture_pack(const std::string& mod_id) {
...
bool recompui::renderer::is_texture_pack_enable_config_option(const recomp::config::ConfigOption& option, bool show_errors) {
    if (option.id == recompui::renderer::special_option_texture_pack_enabled) {
        if (option.type != recomp::config::ConfigOptionType::Enum) {
            if (show_errors) {
                recompui::message_box(("Mod has the special config option id for enabling an HD texture pack (\"" + recompui::renderer::special_option_texture_pack_enabled + "\"), but the config option is not an enum.").c_str());
            }
            return false;
        }
        ...
        if (option_enum.options.size() != 2) {
            if (show_errors) {
                recompui::message_box(("Mod has the special config option id for enabling an HD texture pack (\"" + recompui::renderer::special_option_texture_pack_enabled + "\"), but the config option doesn't have exactly 2 values.").c_str());
            }
            return false;
        }
